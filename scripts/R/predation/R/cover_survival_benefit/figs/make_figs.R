...
source(here::here("R", "cover_survival_benefit", "cover_benefit_funcs.R"))

# file paths and params ---------------------------------------------------

cover_data_path <- here::here("data", "lit_search", "cover_data.csv")
cover_sim_path <- here::here("data", "simulation", "cover_simulation_data.csv")
...
readr::read_csv(cover_data_path) %>%
  ...
  ggplot2::geom_line(ggplot2::aes(dis_to_cover_m, .fitted, color = "red"), alpha = 0.9, size = 1.5, show.legend = FALSE) +
  ggplot2::labs(x = "distance to cover (m)", y = "scaled proportion of observed salmonids")
ggplot2::ggsave(here::here("output", "figs", "cover_benefits", "cover_benefits.jpg"), device = "jpeg")
