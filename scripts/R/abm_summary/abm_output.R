...
source(here("scripts","R","abm_summary","abm_output_functions.R"))

##### Load the data #####

detailed_data_temp <- read.csv(file = here("temporary", "NetLogo", "detailed_pop_output.csv"),
                               skip = 1) %>% 
  mutate(date = ymd(time)) %>% 
  filter(juveniles != 0)
cell_data_temp <- read.csv(file = here("temporary", "NetLogo", "cell_info_output.csv"),
                           skip = 1) %>% 
  mutate(date = ymd(time))
event_data_temp <- read.csv(file = here("temporary", "NetLogo", "events_output.csv"),
                            skip = 1) %>% 
  mutate(date = ymd(time))
...
growth_plot = map(detailed_data_list,
                   ~make_line_plot(data_frame = .x,
                                   x_axis = date,
                                   y_axis = mean_rearing_length_change,
                                   x_lab =  "Date",
                                   y_lab = "Growth (cm)",
                                   title = .x$Species[1])) %>% 
  wrap_plots(ncol = 1, widths = plot_widths,
...
count_v_growth_plot = ggplot(data = detailed_data) +
  theme_classic(base_size = 20) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(x = "Count", y = "Growth (cm)") +
  geom_point(aes(y = mean_rearing_length_change, x = rearers, color = Species),
             size = 2, shape = 1, stroke = 1.5) +
  scale_color_manual(values = cbPalette)

count_v_growth_plot = map(detailed_data_list,
                  ~make_scatter_plot(data_frame = .x,
                                  x_axis = rearers,
                                  y_axis = mean_rearing_length_change,
                                  x_lab =  "Count",
                                  y_lab = "Growth (cm)",
