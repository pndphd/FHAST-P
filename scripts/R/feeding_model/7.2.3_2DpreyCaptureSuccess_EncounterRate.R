...
#Nor does it prepare the arrival of the fish by moving early. And also probably because the prey density was very low in the experiment.
 ...
 nextCapturePrey.row<-function(p.row.start,p.row.end){
  ...
  # p.row.end<-3
   ...
   #2.2.2) Compare the [d,3] (temprary fit Y based on encounter rate) and the [d,15] which is the fit value 
   for (d in p.row.start.plus1:p.row.end){
   if(prey_catch[d,3] > prey_catch[d,15]) { # While the prey fits the ellipsoid, 
     # it is outside the detection range so that the fish will not consider this prey.
     prey_catch[d,3] <- -0.1
      } else (prey_catch[d,3] <- prey_catch[d,15])
   }
   
   
 #  if(prey_catch[d,15] < 0){ # if it doesn't fit I will assign the Y value to -0.1
 #    prey_catch[d,3]<- -0.1
 #  } else if(prey_catch[d,3]>fitPreyEllipsoid){ # although it fits, it is outside of the detection range, I will also assign -0.1
 #    prey_catch[d,3]<- -0.1
 #    else if(prey_catch[d,3]<fitPreyEllipsoid){
 #      prey_catch[d,3]<-prey_catch[d,3]
  #   }
  # }
   
   
   #2.2.2) Assign preys that are outside of the ellipsoid to -0.1 since the fish will not consider it.
   for (d in p.row.start.plus1:p.row.end){
   if (prey_catch[d,15] < prey_catch[d,3]){
     prey_catch[d,3]<- -0.1
      }
   }
  ...
  ##     prey_catch[i,11]<- returnspeed * (d.preyanddetection/preyspeeds) }
   ...
   # Also, give a miss for the ones that do not fit to the ellipsoid
   
   for ( j in p.row.start.plus1:p.row.end){
     if(prey_catch[j,3]<0){
       prey_catch[j,9]<-2 # These preys don't fit to the ellipsoid in the first place.
     }
   }
