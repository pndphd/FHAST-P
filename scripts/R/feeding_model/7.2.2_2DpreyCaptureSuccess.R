...
nextCapturePrey.row<-function(p.row.start,p.row.end){
  ...
  #BTPT<-(100/preydensity+1)/preyspeeds # Between Prey-Time: Amount of time between one prey to the next.
...
Capturesuccess_FHAST2022(preydensity=1, tankdepth=30,
                         preyspeeds=20, fishsize=0.07) 



Capturesuccess_FHAST2022<- function(preydensity,maxpreyconsider,tankdepth,preyspeeds,fishsize){
# Fixed
  numberofprey=1100
  tankwidth<-100
# Varied
...
prey_catch<-matrix(0,numberofprey+0+1,14) # first 10 is for trial (help remove bias from early entry of prey)
...
prey_catch[,1]<-runif(numberofprey+0+1, min=-(tankwidth/2)+0.02,max=(tankwidth/2)-0.02); 
prey_catch[,2]<-runif(numberofprey+0+1, min=1.2,max=tankdepth-0.02); 
...
if(nextCapturePrey.ROW==0){ # if prey is not available over the next several steps, then return towards the origin
  ...
  if(i>1050){ # I am giving ample room to test various prey densities up to 50.
  ...
  # print(prey_catch)
   ...
   if( prey_catch[mpc,3] > (100/preydensity) ){ 
     prey_catch[mpc,3] <- (100/preydensity)
   } else{ prey_catch[mpc,3] <- prey_catch[mpc,3] }
   
   
   # Dist_to_ prey_lateral; 2D distance from fish to the potential prey (in 2D) using Euclidean distance formula
   (prey_catch[mpc,7]<-sqrt((prey_catch[mpc,1]-prey_catch[mpc,4])^2+(prey_catch[mpc,2]-prey_catch[mpc,5])^2) )
   prey_catch
   ...
   prey_catch[i,8]<-Intercept_successdist
   prey_catch
   
   # 1. Capture determination.  Based on the dist to prey destination and fish location
   
   # Decision to see if the incoming prey is even going to be in the detection range of where the fish is in the first place..
   ...
   if( thisfish_ellipsoid_touchY2 > 0 & prey_catch[i,7] < prey_catch[i,8]){ # if the fish is available to go further than the prey destination, it's a catch
     prey_catch[i,9]<-1
   } else { # if not, the fish missed it..because it's out of the 
     prey_catch[i,9]<-2
   }
 ...
 ###########  
   ...
   if(prey_catch[i,9]==1){ # if prey catch is possible (successful), put fish to the prey 2d lateral location
     ...
     prey_catch[i,12]<- prey_catch[i,4] # Original fish spot
   ...
   prey_catch[i+1,8]<-Intercept_successdist
