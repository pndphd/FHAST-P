...
Capturesuccess_Piccolo2007(numberofprey = 50, preydensity=1, fishsize=0.053, 
                           ...
                           dataType="VaryDepth") # or VaryVelocity
                          # When VaryDepth, you can have multiple tankdepth but when VaryVelocity, you should fix to one.

Capturesuccess_Piccolo2007<- function(numberofprey,preydensity,fishsize,tankwidth,tankdepth,dataType){
 
preydensity<-1
numberofprey<-5
fishsize<-0.083
tankwidth=100
tankdepth=c(15,30,45,60) #,30,45,60 cm
tankdepth=30 # for "VaryVelocity" experiment
dataType="VaryVelocity" # or VaryDepth
preyspeeds<-c(29,39,48,54,61) # cm

# (preydensity<-(29.81333*30*((-0.30/j)+0.42))/100) # Piccolo formula of speed & detection distance incorporated in the density conversion
    # 29.8133 is the average density of prey in 1m^3. However, use this formula only when simulating natural systems..Start with a fixed density
if(dataType=="VaryVelocity"){
  model.results<-matrix(0,length(preyspeeds),3)
  colnames(model.results)<-c("velocity","CS_Piccolo","CS_Model")}else if(dataType=="VaryDepth"){
  model.results<-matrix(0,length(tankdepth),3)
  colnames(model.results)<-c("Depth","CS_Piccolo","CS_Model")}


##### VaryVelocity loop ####
if(dataType=="VaryVelocity"){#### Loop start; VaryVelocity ####
for (j in preyspeeds){
  j=29
...
if( prey_catch[1,3] > (120/preydensity) ){ 
  prey_catch[1,3] <- (120/preydensity)
...
#### Important: Determine how long the chamber has to be: 
# While the author varies the prey feeding intervals (ranging from 1-30 seconds), this may complicate 
# many things. Therefore, we will assume that the prey interval is 15 seconds. However, our model is based 
# on the distance perspective which makes things easier in the final stage. Therefore, we would need to
# calibrate what distance is equivalent to those 15 seconds depending on the changing velocities.
# We don't have to stick to 75cm chamber length as long as the fish has enough time to return to its
# origin which is the default condition of the Piccolo et al. 2007.

## 1. Amount of time it takes to attack and return
# furthest distance the fish can go is 0.30meter (highest Z axis) based on the Piccolo et al. 2007
#(interceptionspeed <- (-0.11*(29/100)+0.45)*100) # Piccolo paper reference. converted to cm
#(returnspeed <- (1.39*(29/100)-0.13)*100)# Piccolo paper reference
#timetakenattackreturn<-( 0.30/(interceptionspeed*0.01)) + (0.30/(returnspeed*0.01))
## 2. calculate the distance needed for the chamber
# (timetakenattackreturn*0.61) 
# Conclusion: We will use 1.2 meter !!!



# Feeding time interval (only use for density 1: vary time within 15 seconds interval)
# Piccolo needed this because it didn't want the fish to find a pattern of when the prey are drifting.
# However, our model fish doesn't have that. THerefore, we don't need this random time point between 15 seconds.
# Since I am making a function that is applicable to all prey densities, I will not be using this data.
#feeding_time<-matrix(0,numberofprey+1,3)
#colnames(feeding_time)<-c("randomtime","leftovertimefrom_onestepbefore","preyinterval_fromfeeder")
#feeding_time[,1]<-runif(numberofprey+1,0.01,14.99)
#feeding_time[1,2]<-0

#for (f in 2:(numberofprey+1)){
#  
#  feeding_time[f,2]<- 15-feeding_time[f-1,1]
#}

#for (g in 1:(numberofprey+1)){
#  feeding_time[g,3]<-feeding_time[g,1]+feeding_time[g,2]
#}


# fish new location and capture determination

# April. 29, 2022
# Method for strategic capture:
# Make a column that determines whether they are within the detection area or not namely, detectedgroup or DP.
# the factor that determines the DP is the amount of time between one prey to the next prey.
# this is then affected by how the preys are dispensed from the feeder. 
# When we are testing density 1, we can manipulate the time dispersal for example how we have the 'feeding_time'.
# However, when we are testing more than density 1, we would need to equal the time interval for simplicity and also to maximize the capture success.
# This is because, we are interested in the maximum amount of prey capture given a certain prey density.
# THerefore, we need a column for DP. Use these time interval to determine whether the next prey would be detected while the fish is moving to the first prey.
# From there, we can have another column (CaptureSuccessRate), which is derived from the formulat given by Piccolo et al. 
# The fish will move to the one that has higher capture success.
# But remember this that the cpature success for the next prey is determined by where the current fish is, not where the origin is. So, the fish location has
# to be updated as well....
# The amount of time for the next prey to enter the Detection area is the amount of time the fish was given to move to the first fish.
# The fish then calculates the capture success and compares the two or even more items and decide which one to go for.
# But my concern is ... since the preys are continuously coming in... the DP will need to be continuously updated...
...
for (i in 1:numberofprey) {
 i<-1
  ...
  #Dist from prey to origin: 3D distance from fish to the origin ((0,2,0) in using Euclidean distance formula
  dist_prey_origin<-sqrt((prey_catch[i,1]-fish_originx)^2+(prey_catch[i,2]-fish_originz)^2+(prey_catch[i,3]-fish_originy)^2)
  prey_catch[i,10]<-dist_prey_origin
...
## How far do you return towards the origin? 
  # From here we are trying to figure out how far the fish can return to the origin after attempting an attack.
  # In density 1 to 3, we wouldn't need to worry as much since there is large gap time between one prey to the next.
  # However, for density 4-6 it would become more difficult for the fish to return to its origin due to insufficient time.
    
  #Return distance; considers the time the next prey comes to the detection range
  # 15 seconds is the amount of time prey comes after the next prey.
  # 15/preydensity = amount of time from the next prey to be released from the feeder. This means, after the density 1 experiment,
  # preys are not dispensed every 15 seconds but dependent on the prey density.
  # 120/j= amount of time it takes the released (from feeder) prey to arrive to the fish. This is needed because the start point is the feeder. 
  # The clock is ticking as soon as the prey is released.
  # detection.dist= instead of making a new column with the real detection distance, I am calculating each..and plugging it in the returnmax_dist
  
  # Fitting the XZ coordinates to the ellipsoid to calculate the Y distance. 
  # If it can't be calculated (NA) it means, it's out of bound
  # and it should be considered 0 which means there is not distance and also means that eventually there will be no time to catch it.
  # detection.dist.true.per.row means  how far the fish is out from the 2D plane because this is the fitted Y value of the ellipsoid.
  
  
  detection.dist.true.per.row<-prey.lateral.dist.y.b * sqrt(1-(prey_catch[i,1]^2/detect.width.x.a^2)- ((prey_catch[i,2]-2)^2/detect.height.z.c^2))
  if(is.na(detection.dist.true.per.row)){
    detection.dist.true.per.row<-0
  } else{detection.dist.true.per.row<-detection.dist.true.per.row}
  

  # Seeing how far the fish can return to after the attack.
  # Our assumption is that the preys will be delivered at an equal distance in other words, No variation in time..
  # This makes our estimation and calculation easier in many ways.
  # This is also to see the maximum capture success the fish can have. By evening out the distance,
  # fish doesn't have to worry about encountering multiple prey at once.
  # Disclaimer! The codes here does not perfectly replicate the Piccolo et al. 2007!
  # Reason is because, in the paper the preys are on average 15 seconds apart.
  # Whereas, in my code, the preys are apart based on the prey density (distance) rather than the time.
  # Since, we want to give the fish enough time (say 15seconds on average) to return to its original point we would need to calibrate this time into distances.
  # For example, in the Piccolo experiment, the fish needs around 1.8 seconds to attack and return to its origin in the 0.30 depth experiment.
  # If I calculate the amount of distance it would take the fish to come back in terms of velocity 0.61m/s, it's close to 1.2 meters.
  # All of this process is to match the Piccolo et al. 2007 paper and to accurately test the density 1 condition. 
  # Whereas, these conditions can easily be used to test multiple density effects as well.
  # The reason, my code is ok to assume that it matches the condition with with Piccolo et al. 2007 is because 
  # regardless of the velocity and the prey density(time), it would have ample amount of time to go back to the origin which is 
  # one of the main conditions of Piccolo et al. 2007.
  
  
  if (detection.dist.true.per.row >= 120/preydensity){ #If the next prey has already reached within the detection area don;t return towards origin.
    # Note: it's not preydensity+1. Think about when we only have 1 prey within the 75cm cage and the prey is at the origin plane. 
    # We are looking at <75cm. Count based on when one prey has reached the origin 2D space.
    returnmax_dist<-0
  } else{ # one prey per 15 seconds is the density=1. Considering the time the next prey comes inside the detection range.
    # If I use feeding_time variation, then the density would not work as it becomes difficult to control for the density
    # while also varying the prey delivery time.
    # Therefore, forget about the 15 seconds.
    # Available time= distance between prey/velocity - distance of lateral.y.b/velocity
    # We are using an approximate (the lateral.dist.y.b). This is how long a time the fish can move e.g, until the prey reaches that furthest detection point.
  returnmax_dist <- returnspeed * ( ( (120/preydensity)-prey.lateral.dist.y.b)/j ) #
  }
  if (returnmax_dist < 0){
    returnmax_dist <- 0
  } else{returnmax_dist<-returnmax_dist}
  
  # The distance the fish can return until the next prey reaches its detection area.
  ...
  if (prey_catch[i,7] <= prey_catch[i,8]) { # when fish can catch the prey
    prey_catch[i,12] <- prey_catch[i,1];  # location of the new fish becomes the prey location
    prey_catch[i,13] <- prey_catch[i,2]; 
    if(prey_catch[i,11] >= prey_catch[i,7]){ #if max returning distance is higher, then return to origin. If not stop in between the origin and the prey location
      prey_catch[i+1,4]<- fish_originx
      prey_catch[i+1,5]<- fish_originz
    }else{prey_catch[i+1,4]<- prey_catch[i,1]  + (prey_catch[i,11]/prey_catch[i,7])*(fish_originx-prey_catch[i,1]) ; # fish_originx # # consider prey location + return to the origin with left over time
    prey_catch[i+1,5]<- prey_catch[i,2]  + (prey_catch[i,11]/prey_catch[i,7])*(fish_originz-prey_catch[i,2]); # fish_originy # # consider prey location + return to the origin with left over time
    ...
    prey_catch[i,9] <- 1;} else { # when fish can't catch the prey move towards the prey and then return towards the origin
      prey_catch[i,12] <- prey_catch[i,4] + ((prey_catch[i,3]/j)*interceptionspeed/prey_catch[i,7])*(prey_catch[i,1]-prey_catch[i,4])
      prey_catch[i,13] <- prey_catch[i,5] + ((prey_catch[i,3]/j)*interceptionspeed/prey_catch[i,7])*(prey_catch[i,2]-prey_catch[i,5])
      if(prey_catch[i,11] >= sqrt((prey_catch[i,12]-fish_originx)^2 + (prey_catch[i,13]-fish_originz)^2) ){ #if max returning distance is higher than the dist from fish to origin, then return to origin. If not stop in between the origin and the fish location
        prey_catch[i+1,4]<- fish_originx
        prey_catch[i+1,5]<- fish_originz
        
      }else{fishtorigin<-sqrt((prey_catch[i,12]-fish_originx)^2 + (prey_catch[i,13]-fish_originz)^2)
  ...
  # Fitting PreyY2 to a ellipsoid with the new fish location (prey[i+1,4])
  # PreyY2 changes based on the new location of the fish which is why we put this algorithm at the end and assign it to the next row.
  
  prey_catch[i+1,3]<-prey.lateral.dist.y.b * sqrt(1-((prey_catch[i+1,1]-prey_catch[i+1,4])^2/detect.width.x.a^2)- ((prey_catch[i+1,2]-prey_catch[i+1,5])^2/detect.height.z.c^2))
  prey_catch[i+1,3][is.na(prey_catch[i+1,3])]<-0
  
  # If density increases, the next prey will have arrived in a closer distance from the fish.
  # So, I am forcing to change the Y-distance not to fit to the ellipsoid
  
  if( prey_catch[i+1,3] > (120/preydensity) ){
    prey_catch[i+1,3] <- (120/preydensity)
  } else{ prey_catch[i+1,3] <- prey_catch[i+1,3] }
...
# Total capture success; my model vs Picoolo reference based results
#print(percentcatch<-nrow(prey_catch.m[prey_catch.m$Capture <2,])/numberofprey) # my model result
#(catch_abundance<-nrow(prey_catch.m[prey_catch.m$Capture <2,])) # my model result
# print("Piccolo reference")
#  print(No_of_prey_catch_Piccolo <- (-151.9*(preyspeed/100) + 104.8)/100  ) # Based on Piccolo reference
...
model.results[data.location,3]<- (nrow(captureatvelocity)/numberofprey)*100

#################################################
#######  Vary Depth Loop starts ################

}  } else if(dataType=="VaryDepth"){ #### Loop starts; VaryDepth ####
  for (d in tankdepth){
      ...
      #Sampling prey randomly
      prey_catch[,1]<-runif(numberofprey+1, min=-(tankwidth/2)+0.02,max=(tankwidth/2)-0.02); 
      prey_catch[,2]<-runif(numberofprey+1, min=1.2,max=d-0.02); 
      
      # Estimate max detction distance (formula from 4.5_Detectionrange_parameter.R)
      # Note: the x and y axes are opposite in this script.
      (prey.lateral.dist.y.b<- 100*(0.88915+(-1.59704*(fixedspeed*0.01)^2)+(0.73901*(fixedspeed*0.01))+(0.63983*fishsize^2)+(0.06965*fishsize)) )
      
      (detect.width.x.a<- 100*(0.53269+(0.01084*((fixedspeed*0.01)^2))+(-0.52829*(fixedspeed*0.01))+(-1.50790*fishsize^2)+(-0.22174*fishsize)) ) # Fig.4 Piccolo
      
      (detect.height.z.c<- 100*(0.9202+(0.7394*((fixedspeed*0.01)^2))+(-0.7709*(fixedspeed*0.01))+(-34.5407*fishsize^2)+(2.9616*fishsize)) ) # y value # 41.15282 -0.25872*velocities )# same as the x width  #23.8  # mean of detect.heights
      ...
      # furthest distance fish can reach (at 0.41m/s) in a given time
      # preyspeed <- 29 # cm/s equivalent to water speed
      # preyinterval <- 725 # 10 # cm. Distance between one prey to the next. why was this??. THis is because 4 is in the 72000cm^3 and divided by 4 this is almost 10 cm each..
      
      
      # (fish_detection_dist <- (-0.30*(j/100)+0.42)*100/preydensity) # Piccolo 2007 for Steelheaa. I divided this with the predensity
      
      # (fish_capture_dist <- 8) # cm
      # (fish_search_dist <- fish_detection_dist + fish_capture_dist) # only use when density is only 1.
      
      
      (interceptionspeed <- (0.15*(d*0.01)+0.26)*100 )# Piccolo paper reference. converted to cm
      (returnspeed <- (-0.002*(d*0.01)+0.34)*100 )# Piccolo paper reference
      
      
      # Feeding time interval (vary time within 15 seconds interval)
      feeding_time<-matrix(0,numberofprey+1,3)
      colnames(feeding_time)<-c("randomtime","leftovertime","feedinginterval")
      feeding_time[,1]<-runif(numberofprey+1,0.01,14.99)
      feeding_time[1,2]<-0
      ...
      for (i in 1:numberofprey) {
        ...
        # Dist_to_ prey_lateral
        prey_catch[i,7]<-sqrt((prey_catch[i,1]-prey_catch[i,4])^2+(prey_catch[i,2]-prey_catch[i,5])^2)
        prey_catch
        
        #Dist from prey to origin
        dist_prey_origin<-sqrt((prey_catch[i,1]-fish_originx)^2+(prey_catch[i,2]-fish_originz)^2+(prey_catch[i,3]-fish_originy)^2)
        prey_catch[i,10]<-dist_prey_origin
        
        #Intercept max distance
        (fish_interception_max_dist <- (prey_catch[i,3]/fixedspeed)*interceptionspeed)
        prey_catch[i,8]<-fish_interception_max_dist
        
        
        #Return distance; considers the time the next prey comes to the detetion range
        # 15 seconds is the amount of time prey comes after the next prey (however, this will vary later)
        # 15/preydensity = amount of time from the next prey to be released from the feeder. 
        # 75/j= amount of time it takes the released (from feeder) prey to arrive to the fish. This is needed because the start point is the feeder. The clock is ticking as soon as the prey is released
        # detection.dist= instead of making a new column with the real detection distance, I am calculating each..and plugging it in the returnmax_dist
        
        
        detection.dist.true.per.row<-prey.lateral.dist.y.b * sqrt(1-(prey_catch[i,1]^2/detect.width.x.a^2)- (prey_catch[i,2]^2/detect.height.z.c^2))
        if(is.na(detection.dist.true.per.row)){
          detection.dist.true.per.row<-0
        } else{detection.dist.true.per.row<-detection.dist.true.per.row}
        
        
        if (detection.dist.true.per.row >= 120/preydensity){
          returnmax_dist<-0
        } else{ returnmax_dist <- returnspeed * ( (feeding_time[i,3]/preydensity)-((75/preydensity)/fixedspeed)-((75/preydensity)-detection.dist.true.per.row)/fixedspeed ) # one prey per 15 seconds is the density=1. Considering the time the next prey comes inside the detection range.
        }
        if (returnmax_dist < 0){
          returnmax_dist <- 0
        } else{returnmax_dist<-returnmax_dist}
        
        # 시간인데..어던 시간이냐면..다음 먹이가 올때까지의 시간임.. 그래서 여기에 밀도개념을 적용 시키면 돼...
        prey_catch[i,11]<-returnmax_dist
        ...
        if (prey_catch[i,7] <= prey_catch[i,8]) { # when fish can catch the prey
          ...
          prey_catch[i,9] <- 1;} else { # when fish can't catch the prey move towards the prey and then return rowards the origin
            prey_catch[i,12] <- prey_catch[i,4] + ((prey_catch[i,3]/fixedspeed)*interceptionspeed/prey_catch[i,7])*(prey_catch[i,1]-prey_catch[i,4])
            prey_catch[i,13] <- prey_catch[i,5] + ((prey_catch[i,3]/fixedspeed)*interceptionspeed/prey_catch[i,7])*(prey_catch[i,2]-prey_catch[i,5])
            if(prey_catch[i,11] >= sqrt((prey_catch[i,12]-fish_originx)^2 + (prey_catch[i,13]-fish_originz)^2) ){ #if max returning distance is higher than the dist from fish to origin, then return to origin. If not stop in between the origin and the fish location
              prey_catch[i+1,4]<- fish_originx
              prey_catch[i+1,5]<- fish_originz
      ...
      data.location <- which(tankdepth==d)
      captureatvelocity<-prey_catch.m[prey_catch.m$Capture <2,]
      
      model.results[data.location,1]<-d
      model.results[data.location,2]<- -0.05*(d*0.01) +39.9 # Formula from Piccolo et al.2007
      model.results[data.location,3]<- (nrow(captureatvelocity)/numberofprey)*100
    } 
} #End of Depth loop
...
# calculate the capture rate per distance and compare with thte Hill and Grossman 1993.
# Subtract the prey location from the fish location!! 
