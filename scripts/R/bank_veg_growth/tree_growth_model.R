...
grow_tree <- function(height_m = NULL,
  ...
  growth_parms = growth_parms %>% 
    mutate(specise = str_to_lower(species))
  ...
  radius_increase = diameter_f - diameter_0
  
  # return and convert to m
  return(data.frame(height_f, radius_increase))
}
...
find_diameter = function(dbh_in, parms_in){
 ...
 if(parms_in$eq_name == "lin"){
   diameter = parms_in$a + parms_in$b * dbh_in
    ...
    diameter = parms_in$a + parms_in$b * dbh_in + parms_in$c *dbh_in^2 + d * dbh_in^3
...
# Do the calculations
new_canopy <- canopy_shape %>% 
  add_column(!!!cols[setdiff(names(cols), names(.))]) %>% 
  rowwise() %>% 
  mutate(growth = grow_tree(height_m = height,
                                species = species,
                                time = 1,
                                growth_parms = tree_growth_parms))