...
patches-own [
  ...
  sav
  lwd
  shade               ; The shade value of a cell currenlty just july
  veg
  trees
...
to setup
  ...
  set flow_input_csv csv:from-file "../../../temporary/NetLogo/Depth_Velocity_Data_Input_20_600.csv"   ; Read in the depth CSV
  set shape_input_csv csv:from-file "../../../temporary/NetLogo/Shape_Data_Input_20_600.csv"  ; Read in the shape file
  ...
  set sav_column (position "sav" (item 0 shape_input_csv))
  set lwd_column (position "lwd" (item 0 shape_input_csv))
  set trees_column (position "trees" (item 0 shape_input_csv))
  set veg_column (position "vegetation" (item 0 shape_input_csv))
  set fine_column (position "fine" (item 0 shape_input_csv))
  set rock_column (position "rock" (item 0 shape_input_csv))
  set shade_column (position "shade_7" (item 0 shape_input_csv))
...
;    [random 3 - 1 + today_index])                           ; Else add between -1 add 1 to it
;  set today_index new_index
  ...
  ask patches [
    ...
    (ifelse
      ...
      background_display = "lwd" [
        if (today_depth > 0)[
          set pcolor palette:scale-gradient palette:scheme-colors "Sequential" "YlGnBu" 9 lwd 0 1]
      ]
      background_display = "sav" [
        if (today_depth > 0)[
          set pcolor palette:scale-gradient palette:scheme-colors "Sequential" "PuRd" 9 sav 0 1]
      ]
      background_display = "vegetation" [
        if (today_depth > 0)[
          set pcolor palette:scale-gradient palette:scheme-colors "Sequential" "BuGn" 9 veg 0 1]
      ]
...
background_display
background_display
"depth" "velocity" "sav" "lwd" "vegetation" "shade"
5
