...
globals [
  ...
  shade_column ; The column that has the information on percent shade
  canopy_column ; The column that has the information on percent canopy cover

  ;; Fish variables
  fish_params_csv ; The fish parameter input file
  paired_param_table ; A tabel with parametes and values for fish
  species_list ; List of fish species
  energy_densities ; THe energy density of the fish tussue
  exits ; A list of fish exiting the reach
  strands ; A list of fish stranding

  ;; Predator variables
  pred_lengths_csv ; Input file with lists of predator lengths
  pred_log_params_csv ; Input file with model parameters to calculate predator habitat ratings
  total_preds ; The total number of predators in the system
  pred_input_file_csv ; csv file with some default params for pred counts
  conv ; factor for converting from the inputed preds per area measure to preds per sq. m
  preds_per_area ; number of predator per unit area
]

;; Define variables that the patches own
patches-own [
  ...
  wetted_area ; the total area wetted
  ...
  ; shade_orig ; A place holder to store old shade value
...
;; Define variables that the fish own
turtles-own [
  ;; Species
  number ; The assigned number for this species
  species ; The species name
  species_id ; A number for the species

  ;; Physological parameters
  mass ; Mass
  energy_density ; The fish energy density
...
to setup
  ...
  set pred_lengths_csv csv:from-file "../../../scripts/R/predation/output/pred_length_dist_params.csv" ; Read in csv with lists of predator lengths
  set pred_log_params_csv csv:from-file "../../../scripts/R/predation/output/pred_log_params_netlogo.csv" ; Read in csv with lists of predator lengths
  set pred_input_file_csv csv:from-file "../../../temporary/NetLogo/predator_input_file.csv" ; Read in the predator input csv
  ...
  set x_flow (position "lat_dist" (item 0 flow_input_csv))
  ...
  ;; Put the fish parameters into a table
  set_fish_parameters
  ...
  ;; set binary substrate value for each patch
  set_substrate_value

  ;; set predator input params
  set conv item 1 (item 1 pred_input_file_csv)
  set preds_per_area item 0 (item 1 pred_input_file_csv)
end
...
;; Get the flow values from the depth file
to set_flow_values
  ; Get the flow values form the daily input
  set daily_flow_values but-first (map [n -> item flow_column n ] daily_input_csv) ; Read in all the x positions
  let flow_header item 0 flow_input_csv ; Read in all the column headers
...
to set_fish_parameters
  let parameter_names but-first (map [n -> item 0 n ] fish_params_csv)
  let parameter_values but-first (map [n -> but-first n] fish_params_csv)
  ; combine the last to to make a paired list then a table
  let paired_param_list (map [ [ a b ] -> ( list a b ) ] parameter_names parameter_values)
  set paired_param_table table:from-list paired_param_list
end

;; Assign each of the cells all it's flow values
to set_patch_flow_values
;; Assign all the patches list of depths to have 0 to rep no data
  ask patches [
  ...
  foreach (but-first flow_input_csv) [n ->
    ...
    ask patch ((item x_flow n) / (resolution)) ((item y_flow n) / (resolution)) [
      ...
      foreach but-first flow_values [m ->
        ...
        let m_column (position (word "mean.D" m) (item 0 flow_input_csv))
...
;; Assign each of the cells all it's shade values
...
;; Assign substrate value to patches, which is used for predator calculations
to set_substrate_value
ask patches [
...
to set_flow
  ; Caclulate the flow values
  set flow (item ticks daily_flow_values) ; get todays flow value
  ...
  let high_flow min filter [i -> i >= flow] flow_values ; find the closest flow which is greater than
  let low_flow max filter [i -> i < flow] flow_values ; find the closest slow which is less than
  ...
  ask patches [
    ...
    set today_depth (low_depth * (1 - flow_fraction) + high_depth * (flow_fraction))
    if today_depth <= 0 [set today_depth 0]
    ; Do the same for velcotity
    ...
    set today_velocity (low_velocity * (1 - flow_fraction) + high_velocity * (flow_fraction))
    ...
    set wetted_area wetted_fraction * area
  ]
end
...
to place_predators
  ...
  set total_preds round ((sum [wetted_area] of patches) / conv * preds_per_area )
  ...
  ask patches [
    ...
    [ 
      ...
      foreach (range 1 (length(pred_log_params_csv)))
      [ n ->
        let model_pars but-first item n pred_log_params_csv
        let hab_vars (list 1 shade_binary veg wood today_depth today_velocity substrate)
        let exponent -1 * (sum (map * model_pars hab_vars))
        let model_prediction 1 / (1 + e ^ (exponent))
    ...
    [
      ...
      set hab_rating n-values (length but-first pred_log_params_csv) [0]
    ]
  ]
  ...
  foreach (range 0 length(but-first pred_log_params_csv)) [n ->
    set total_hab_rating lput sum-item n ([hab_rating] of patches) total_hab_rating
  ]
  print total_hab_rating
end
...
;; Set the color of the batches based on user input
to color_patches
  ask patches [
    ...
    (ifelse
      background_display = "depth" [
        if (today_depth > 0)[
          ...
          ;set pcolor palette:scale-gradient palette:scheme-colors "Divergent" "Spectral" 9 today_depth 0 (max_depth)]
      ]
      ...
      background_display = "hab rating" [
        if (today_depth > 0)[
          set pcolor palette:scale-gradient palette:scheme-colors "Sequential" "YlGnBu" 9 hab_rating 0 1]
      ]
    )
  ]
end
  ...
  loop[
    ...
    if item (lifestage_column - 1) todays_fish = "adult" [
      create-adults (item (number_column - 1) todays_fish )[
        ...
        set energy_density (item species_id (table:get paired_param_table "energy_density"))
        set size 10
        move-to one-of wet_patches     ; Place the turtle in any
        set color blue
    ...
    ask cur_patch [
      ask neighbors [
        ...
        [
          ...
          if (has_visited = false) or (cost < path_to_here_cost)
          [
...
background_display
background_display
"depth" "velocity" "veg" "wood" "shade" "wetted fraction" "hab rating"
0
