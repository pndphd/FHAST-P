...
globals [
  ...
  veg_column          ; The location index for value of a cell read in from the flow CSV
  cover_column        ; The location index for cover of a cell read in from the flow CSV
  food_column         ; The location index for zone of a cell read in from the flow CSV
  shade_column        ; The location index for shade of a cell read in from the flow CSV
...
to setup
  ...
  set flow_input_csv csv:from-file "../../../temporary/NetLogo/Depth_Velocity_Data_Input_20_500.csv"   ; Read in the depth CSV
  set shape_input_csv csv:from-file "../../../temporary/NetLogo/Shape_Data_Input_20_500.csv"  ; Read in the shape file
  ...
  set food_column (position "food" (item 0 shape_input_csv))
  set shade_column (position "shade_7" (item 0 shape_input_csv))
  set cover_column (position "cover" (item 0 shape_input_csv))
...
to set_patch_shape_values
  ...
  foreach (but-first shape_input_csv) [n ->
    ask patch ((item x_shape n) / (resolution)) ((item y_shape n) / (resolution)) [
      ...
      set zone (item food_column n)
      set value (item veg_column n)
...
background_display
background_display
"depth" "velocity" "cover" "food" "vegetation" "shade"
5
