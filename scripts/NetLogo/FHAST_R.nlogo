...
globals [
  ...
  pred_success       ; Probablity of a predator succeding if it encounters a prey
...
to edit_inputs
  r:clear
  let wrk_dir (word format_path pathdir:get-model-path "/../..")
  r:eval (word "setwd('" wrk_dir "')")
  r:eval "library(here)"
  r:eval "library(shiny)"
  r:eval "library(shinyFiles)"
  r:eval "shiny::runApp('scripts/ui')"
end
...
to preprocess
  ...
  r:interactiveShell
  r:eval "library(here)"
  r:eval "source(here('scripts','main','run_setup.R'), echo = TRUE)"
  r:eval "source(here('scripts','daily_inputs','make_enviro_input_file.R'), echo = TRUE)"
  r:eval "source(here('scripts','daily_inputs','make_fish_input_file.R'), echo = TRUE)"
  r:eval "source(here('scripts','cover','make_preview_map.R'), echo = TRUE)"
  r:eval "source(here('scripts','river_grid','grid_maker.R'), echo = TRUE)"
  r:eval "source(here('scripts','shade','calculate_shade.R'), echo = TRUE)"
  r:eval "source(here('scripts','depth_velocity','sample_raster.R'), echo = TRUE)"
  r:eval "source(here('scripts','cover','sample_shapes.R'), echo = TRUE)"
end
...
to setup
  ...
  set habitat_params_csv csv:from-file "../../temporary/habitat.csv"                           ; Read in general habitat params
...
;; CHange format of paths for R
to-report format_path [in_path]
  let numbers (range 0 length in_path)
  foreach numbers [ x ->
    if item x in_path = "\\" [
      set in_path replace-item x in_path "/"
    ]
  ]
  let new_path in_path
  report new_path
end
...
to go
  ...
  [
    ...
    r:interactiveShell
    update_output
    r:eval "source(here('scripts','main','run_post.R'), echo = TRUE)"
...
;      ]
  ...
  if exit_status != 1 or strand_status != 1 [ ; If the fish have not stranded or exited the river, they grow and deplete destination resources
...
to set_survival_prob
  ...
  ask wet_patches [
    ...
    set small_survival_prob 1 - pred_success + (pred_success * (1 - ((1 - turb_survival_bonus) * (1 - small_cover_bonus))))
  ]
    ...
    ;[
      ...
      if item n total_hab_rating > 0 [
       ...
       ; let sigma item 1 butfirst item n butfirst pred_length_dist_params
        ...
        let pred_length_list filter [i -> i >= 15] (n-values temp_pred_num [x -> log-normal item n meanlog item n sdlog ])
...
to find_potential_destination_cells
  ...
  set patch_radius (((item species_id move_dist_a) * (f_length ^ (item species_id move_dist_b)))) / resolution ; Number of patches within the maximum distance that a fish can travel. Essentially the "radius" in cm converted to meters and divided by the resolutio
...
to calculate_mortality_risk
  ...
  ask wet_cells_in_radius [
    ...
    [
      set fish_death_aq_pred_prob calc-pred-mortality small_survival_prob encounter_prob max_prey_length ([f_length] of myself) num_preds
    ]
...
; If there are no valid cells that the fish can drift to, it drifts out of the river or strands
  if any? random_drift_downstream_cells = FALSE [ print "no random drift downstream cells"
...
to calculate_net_energy_of_drift_cells
  ...
  ;print "calc net energy of drift cells"
...
to grow
  ...
  if exit_status = 1 and ycor = reach_end [
    die] ; If the drifter has reached the end of the reach and hasn't died, it is removed from the system but doesnt get added to the dead count
...
to-report calculate_patch_survival [fish]
  ...
  [
    ...
    [
      set fish_survival 1 - (calc-pred-mortality small_survival_prob encounter_prob max_prey_length [f_length] of fish num_preds)
    ]
  ]
...
;; Find possible a pathable destination at the specified y of length up
; to the move_dist.
        ...
        [
          ...
          [
            ...
            [
              ...
              if (is_valid_destination fish destinations) ; and (pycor < [ ycor ] of fish) may add in later?
              [
...
to-report calc-pred-mortality [#survival_prob #encounter_prob #max_prey_length #prey_f_length #num_preds]
  ...
  if #max_prey_length < (#prey_f_length)  ; f_length is in cm but the model that calcs max_prey_length uses mm
  [
    report 0
  ];
...
to survive
 ...
 ; print "survive"
  ...
  ifelse not empty? path_survival_list [ ; If fish has a path survival list because it's a drifter, we use the two values in the list to determine whether it dies
    ...
    foreach path_survival_list [n -> ;Compare each item in the path survival list to a random number to see if the fish dies in either path
...
;  ]
  ...
  ;[
    ;set dead_fish_count_table lput 1 dead_fish_count_table
    ;print "im dead"
  ;]
  if is_migrant = true and ycor = reach_end [
    save_event "migrant exited river"
    die
  ] ; If the migrant has reached the end of the reach and hasn't died, it is removed from the system but doesnt get added to the dead count
...
to build_output_file_named [a_file_name]
  ...
  [
    set detailed_population_outfile_name (word "../../temporary/detailed_pop_output.csv")
  ...
  [
    set cell_info_outfile_name (word "../../temporary/cell_info_output.csv")
  ...
  [
    set fish_events_outfile_name (word "../../temporary/events_output.csv")
    ...
    ; lines in reverse order.
    set fish_events_list fput "time, Species, id, x_pos, y_pos, length , length change, mass, previous_condition, condition, growth, percent_daily_growth, max_swim_speed, cmax, swim_speed, overall_outmigration_prob, cell_available_wet_area, in_shelter, is_drifter, is_migrant, drifter_history, starving?, event, temperature, flow, velocity, depth, distance_to_cover, available_velocity_shelter, passive_metab_rate, active_metab_rate, total_metab_rate, daily_intake, daily_energy_intake, total_net_energy, path_survival, prob_of_surviving_predation_of_destination" fish_events_list
...
background_display
background_display
"veg" "wood" "depth" "velocity" "shade" "predator habitat rating" "wetted fraction" "available velocity shelter" "none"
2
