...
source(here("scripts","abm_summary","abm_output_functions.R"))

##### Load the data #####

detailed_data_temp <- read.csv(file = here("temporary", "detailed_pop_output.csv"),
                               skip = 1) %>% 
  mutate(date = ymd(time)) %>% 
  filter(juveniles != 0)
cell_data_temp <- read.csv(file = here("temporary", "cell_info_output.csv"),
                           skip = 1) %>% 
  mutate(date = ymd(time))
event_data_temp <- read.csv(file = here("temporary", "events_output.csv"),
                            skip = 1) %>% 
  mutate(date = ymd(time))
